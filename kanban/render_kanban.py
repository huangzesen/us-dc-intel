...
body.light {{ --bg:#f6f8fc; --card:#ffffff; --line:#dde3ee; --txt:#1a2233; --mut:#5b6b8c; --acc:#2f6fe4; }}
...
body.light .bar {{ background:#e8ecf4; }}
...
body.light .card.done {{ color:#0d7a3f; }}
...
function setTheme(t) {{
  ...
  try {{ localStorage.setItem('kanban-theme', t); }} catch(e) {{}}
}}
try {{ var saved = localStorage.getItem('kanban-lang'); if (saved==='zh' || saved==='en') setLang(saved); }} catch(e) {{}}
try {{ var savedTheme = localStorage.getItem('kanban-theme'); if (savedTheme==='light') setTheme('light'); }} catch(e) {{}}
</script>
</body>
</html>'''
