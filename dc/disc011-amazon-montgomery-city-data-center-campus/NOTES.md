...
## 2026-08-11（fix pass：mcmo.us URL 传输方式修复）
- 独立审查 spot-check 报 https://mcmo.us/amazon/ 连接被拒绝（TCP 443 connection refused；DNS 仍解析到 198.209.253.17）。重试后确认整个 mcmo.us 主机 443 无法连接（HEAD/GET 均失败，包括三个 Project Green PDF）。
- 网站本身尚在运行：http://mcmo.us/amazon/ 返回 200，标题“Montgomery County, MO Data Center - Project Green (Amazon Web Services)”，页面仍列出 Resolution Plan for Industrial Development、Chapter 100 Bond Order、Infrastructure Development & Funding Agreement、site plan、Cost Benefit Analysis 等材料；三个 PDF 经 http 访问均返回 200。
- 处理：data.json `sources` 中 4 个 mcmo.us URL 均已将 `https://` 更改为 `http://`（同一官方源、同内容、可访问），并在 data.json `history` 记录更换。若后续 mcmo.us 恢复 HTTPS，可将链接恢复为 https。
